...
def executing_cluster_diagnostic_checks_job(corev1_api_instance, batchv1_api_instance, helm_client_location,
    ...
    try:
        ...
        # For Winfield the value will be of the form edgeartifacts.edgeacr.<CustomerFQDN>/path/clusterdiagnosticchecks:0.2.1 
        # e.g "edgeartifacts.edgeacr.autonomous.cloud.private/azurearck8s/helmchart/stable/clusterdiagnosticchecks:0.2.1"
        if os.getenv('WINFIELD_DIAGNOSTIC_CHECKS_REGISTRY_PATH'):
            diagnostic_checker_registry_path = os.getenv('WINFIELD_DIAGNOSTIC_CHECKS_REGISTRY_PATH')
...
def helm_install_release_cluster_diagnostic_checks(chart_path, location, http_proxy, https_proxy, no_proxy, proxy_cert,
                                                   azure_cloud, kube_config, kube_context, helm_client_location, arm_metadata
    ...
    # dataplane endpoints from ARM Metadata is only available in Winfield environments
